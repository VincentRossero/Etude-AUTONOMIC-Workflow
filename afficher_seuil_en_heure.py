...
raw_tco2, srate = physio.read_one_channel('EEG_121.TRC',format='micromed', channel_name='tCO2+')
raw_Spo2, srate = physio.read_one_channel('EEG_121.TRC',format='micromed', channel_name='SpO2+')

start_time = '21:35:00'
start_time_seconds = sum(x * int(t) for x, t in zip([3600, 60, 1], start_time.split(':')))
time_offset = start_time_seconds


time = np.arange(0, raw_tco2.size/srate, 1 /srate) + time_offset
...
temps = convert_seconds_to_hh_mm_ss (556)
print (temps)
